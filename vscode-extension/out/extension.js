...
class SSLExtension {
    ...
    activate() {
        // Register commands
        this.context.subscriptions.push(vscode.commands.registerCommand('ssl.secureDocument', () => this.secureDocument()), vscode.commands.registerCommand('ssl.validateSymbols', () => this.validateSymbols()), vscode.commands.registerCommand('ssl.showSecurityReport', () => this.showSecurityReport()), vscode.commands.registerCommand('ssl.addCustomSymbol', () => this.addCustomSymbol()), vscode.commands.registerCommand('ssl.exportReport', () => this.exportReport()));
        // Register event handlers
        this.context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((document) => this.onDocumentSave(document)), vscode.window.onDidChangeActiveTextEditor((editor) => this.onActiveEditorChange(editor)));
    ...
    async secureDocument() {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Document secured! Coverage: ${result.validation_report.symbol_coverage}, ` +
    ...
    async showSecurityReport() {
        ...
        if (this.securityPanel) {
            ...
            this.securityPanel = vscode.window.createWebviewPanel('sslSecurityReport', 'SSL Security Report', vscode.ViewColumn.Beside, {
                enableScripts: true,
    ...
    async callSSLBackend(text, validateOnly = false, action = 'secure', params = {}) {
    ...
    updateStatusBarWithReport(report) {
...
exports.SSLExtension = SSLExtension;
